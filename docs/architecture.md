...
Read this together with [`../AGENTS.md`](../AGENTS.md) and [`project-direction.md`](project-direction.md). Those documents explain the project trajectory and why these boundaries are intentional.

## Goal

Keep the engine generic and the game content replaceable without turning the MVP into a plugin framework.

The architecture should optimize for a complete, understandable game first and reusable abstractions second. Generalize when a concrete bundle or repeated implementation demonstrates the need.
...
In particular, avoid replacing neutral engine terms with words such as realm, kingdom, dynasty, faction, house, court, or empire. Those may be excellent **bundle/UI labels** but are not universal engine concepts.
...
## Server and static demo

The normal architecture is server-authoritative. The GitHub Pages version is a browser-local demo adapter because a static host cannot run the Fastify/WebSocket service.

Do not let static mode become a second rules implementation. Both paths should continue to share the same rules package and UI behavior wherever possible.
...
```text
human-readable source data -> validation/compilation -> disposable derived runtime data
```

Generated data should not become the hand-edited source of truth.
